...
if __name__=="__main__":
    ...
    for i in range(0, 200):
        ...
        total_reward = 0
        steps = 0
        X_data_part = np.ndarray((1, input_shape[0], input_shape[1], input_shape[2]), dtype=np.float)
        while True:
            ...
            if K.image_data_format() == 'channels_first':
                processed_image=np.swapaxes(np.swapaxes(processed_image, 1, 2), 0, 1)
            X_data_part[0] = processed_image
            X_data_part[0] = X_data_part[0]/255
    
            # Get the model to make a prediction
            a = model.predict_classes(X_data_part)
            a = a[0]
